...
describe('Testes de potência', () => {
    ...
    //Potência com expoente negativo usa a regra a**(-b) = (1/a)**b = 1/a**b para cálculo
    it('Deve realizar a potência de 2 por -2 resultando em 0.25', () => {
        let resultado = Calculadora.pot(2, -2);
        expect(resultado).to.be.eq(0.25); 
    });
    //Quando a base é negativa, se o número for par o resultado será positivo, se o número for impar o resultado será negativo
...
describe('Testes de radiciação', () => {
    ...
    //A raiz de um número negativo não é possível de ser calculada = NaN
    it('Deve realizar a radiciação de -81 com índice 2 resultando em 9', () => {
        let resultado = Calculadora.rad(-81, 2);
...
describe('Teste de cálculo de média simples de n números', () => {
    ...
    it('Tendo os valores 10, 10, 10, 10, 10 e 10 resultando na média 10', () => {
...
describe('Teste de cálculo de média simples informando se o aluno foi aprovado ou reprovado', () => {
    //Verificar se a validação da função mediaFinal funciona aqui também
    it('Tendo os valores "cinco", 7, 8 e 8 a média não poderá ser calculada, resultando em uma mensagem de erro pois foi inserida uma string', () => {
        let resultado = Calculadora.aprov('cinco', 7, 8, 8);
        expect(resultado).to.be.eq('ERRO: Insira um NÚMERO entre 0 e 10');
    });
    it('Tendo os valores -7, 7, 9 e 8 a média não poderá ser calculada, resultando em uma mensagem de erro pois foi inserido um valor negativo', () => {
        let resultado = Calculadora.aprov(-7, 7, 9, 8);
        expect(resultado).to.be.eq('ERRO: Insira um VALOR entre 0 a 10');
    });
    it('Tendo os valores 18, 5, 9 e 7 a média não poderá ser calculada, resultando em uma mensagem de erro pois foi inserido um valor acima de 10', () => {
        let resultado = Calculadora.aprov(18, 5, 9, 7);
        expect(resultado).to.be.eq('ERRO: Insira um VALOR entre 0 a 10');
    });

    //Verificar se um aluno com notas baixas será reprovado
    it('Tendo as notas 5, 2, 3, 6 a média é 4 resultando em "Reprovado"', () => {
        let resultado = Calculadora.aprov(5, 2, 3, 6);
        expect(resultado).to.be.eq('Reprovado');
    });

    it('Tendo as notas 2, 3.5, 7 e 8.9 a média é 5.35, resultando em "Reprovado"', () => {
        let resultado = Calculadora.aprov(2, 3.5, 7, 8.9);
        expect(resultado).to.be.eq('Reprovado');
    });

    //Verificar se um aluno com média > 6 será realmente aprovado

    it('Tendo as notas 6, 9.5, 10 e 10 a média é 8.875, resultando em "Aprovado"', () => {
        let resultado = Calculadora.aprov(6, 9.5, 10, 10);
    ...
    it('Tendo as notas 2, 2, 2 e 2, a média é ' + Calculadora.mediaFinal(2, 2, 2, 2) + ', resultando em "' + Calculadora.aprov(2, 2, 2, 2) + '"', () => {
        let resultado = Calculadora.aprov(2, 2, 2, 2);
        expect(resultado).to.be.eq('Reprovado');
    });
});